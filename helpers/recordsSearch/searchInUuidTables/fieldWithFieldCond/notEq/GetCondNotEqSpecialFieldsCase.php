...
namespace Mnemesong\OrmTestHelpers\recordsSearch\searchInUuidTables\fieldWithFieldCond\notEq;

use Mnemesong\Fit\Fit;
use Mnemesong\OrmTestHelpers\recordsSearch\abstracts\RecordsSearchTestCase;
use Mnemesong\Structure\Structure;

class GetCondNotEqSpecialFieldsCase extends RecordsSearchTestCase
{
    public function __construct()
    {
        $this->cond = Fit::field('name')->field('!=', 'nick')->asStr();
        $this->selectFields = ['name', 'nick'];
    }
    ...
    {
        return [
            self::buildShort('Volodya', 'Volz134'),
            self::buildShort('Sofie', 'Sofz123'),
            self::buildShort('Marz23', 'Mary'),
            self::buildShort('Jazz23', 'Jakob'),
            self::buildShort('Simona', ''),
            self::buildShort('', 'Simon'),
            self::buildShort(null, 'Elizabeth'),
            self::buildShort('George', null),
        ];
    }
     ...
     * @return Structure
     */
    protected static function buildShort(?string $name, ?string $nick): Structure
    {
        return new Structure(['name' => $name, 'nick' => $nick]);
    }
}